...
class Auth extends \ba\Auth
{
     ...
     * @var string[] 允许输出的字段
     */
    protected $allowFields = ['id', 'username', 'nickname','product_id','start_time','end_time','num', 'email', 'mobile', 'avatar', 'gender', 'birthday', 'money', 'score', 'jointime', 'motto', 'lastlogintime', 'lastloginip'];
    ...
    public function register(string $username, string $password, string $mobile = '', string $email = '', int $group = 1, array $extend = []): bool
    {
        ...
        $data = [
            ...
            'salt'          => $salt,
            'status'        => 'enable',
            'num'           => get_sys_config('reg_num'), //注册赠送次数
        ];
